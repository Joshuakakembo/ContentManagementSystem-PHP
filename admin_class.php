...
class Action
{
	...
	function save_products()
	{
		extract($_POST);
		if (!empty($content)) {
			$save = file_put_contents('products.html', $content);
			if ($save)
				return 1;
		} else {
			$fh = fopen('products.html', 'w');
			fclose($fh);
			return 1;
		}
	}
	function save_partnerships()
	{
		extract($_POST);
		if (!empty($content)) {
			$save = file_put_contents('partnerships.html', $content);
			if ($save)
				return 1;
		} else {
			$fh = fopen('partnerships.html', 'w');
			fclose($fh);
			return 1;
		}
	}
